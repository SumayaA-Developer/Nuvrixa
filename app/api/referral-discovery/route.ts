...
import { referralSections } from "@/lib/referral-discovery";
...
export async function POST(request: Request) {
  try {
    ...
    const required = referralSections.flatMap((section) => section.fields.filter((field) => field.required)).filter((field) => !present(answers[field.key]));
    ...
    const reference = `NUV-REF-${submittedAt.slice(0, 10).replaceAll("-", "")}-${randomUUID().slice(0, 8).toUpperCase()}`;
    ...
    const rows = referralSections.map((section) => `<h3>${escapeHtml(section.id + ". " + section.title)}</h3>${section.fields.map((field) => `<p><strong>${escapeHtml(field.label)}</strong><br>${escapeHtml(display(answers[field.key]))}</p>`).join("")}`).join("");
    ...
    const note = `<h2>Nuvrixa Referral Discovery</h2><p><strong>Reference:</strong> ${escapeHtml(reference)}<br><strong>Submitted:</strong> ${escapeHtml(submittedAt)}</p>${rows}${fileRows}`;
